...
class SDSD_Bilevel_eventDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(SDSD_Bilevel_eventDataset, self).__init__()
        self.opt = opt
       ...
       # self.center_cropped_height = opt['gt_size']
        ...
        normal_subdir = opt['normal_subdir']
        low_subdir = opt['low_subdir']
        events_subdir = opt['events_subdir']
        events_denoising_subdir = opt['events_denoising_subdir']

        normal_rel = Path(normal_subdir).relative_to(root) if Path(normal_subdir).is_absolute() else Path(normal_subdir)
        low_rel = Path(low_subdir).relative_to(root) if Path(low_subdir).is_absolute() else Path(low_subdir)
        events_rel = Path(events_subdir).relative_to(root) if Path(events_subdir).is_absolute() else Path(events_subdir)
        events_denoising_rel = Path(events_denoising_subdir).relative_to(root) if Path(
            events_denoising_subdir).is_absolute() else Path(events_denoising_subdir)
        ...
        ev_npzs = []
        ev_denoise_npzs = []
        # 遍历 dataset_root 下的所有子文件夹（场景文件夹）
        scene_folders = sorted([d for d in root.iterdir() if d.is_dir()])

        for scene_folder in scene_folders:
            # 构建当前场景的各个子文件夹路径
            scene_normal_folder = scene_folder / normal_rel.name
            scene_low_folder = scene_folder / low_rel.name
            scene_events_folder = scene_folder / events_rel.name
            scene_events_denoise_folder = scene_folder / events_denoising_rel.name

            # 检查文件夹是否存在
            if scene_normal_folder.exists() and scene_low_folder.exists():
                # 收集当前场景的图像
                scene_low_pngs = sorted([p for p in scene_low_folder.iterdir() if p.suffix.lower() == '.png'])
                scene_normal_pngs = sorted([p for p in scene_normal_folder.iterdir() if p.suffix.lower() == '.png'])

                low_pngs.extend(scene_low_pngs)
                normal_pngs.extend(scene_normal_pngs)

                # 如果需要处理事件数据
                if self.is_split_event:
                    if scene_events_folder.exists():
                        scene_ev_npzs = sorted([p for p in scene_events_folder.iterdir() if p.suffix.lower() == '.npz'])
                        ev_npzs.extend(scene_ev_npzs)

                    if scene_events_denoise_folder.exists():
                        scene_ev_denoise_npzs = sorted(
                            [p for p in scene_events_denoise_folder.iterdir() if p.suffix.lower() == '.npz'])
                        ev_denoise_npzs.extend(scene_ev_denoise_npzs)

        # 重新排序以确保全局有序
        low_pngs = sorted(low_pngs)
        normal_pngs = sorted(normal_pngs)

        ev_npzs = sorted(ev_npzs)
        ev_denoise_npzs = sorted(ev_denoise_npzs)

        n = min(len(low_pngs), len(normal_pngs))

        if n == 0:
            raise RuntimeError(f'No png files found in scene folders under {root}')
        ...
        self.ev_paths = [str(p) for p in ev_npzs[:n]]
        self.ev_denoise_paths = [str(p) for p in ev_denoise_npzs[:n]]
        self.ev_denoise_list = [p.name for p in ev_denoise_npzs[:n]]
        self.ev_list = [p.name for p in ev_npzs[:n]]  # events_name
        # #处理数据字段
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            event_lq = self._generate_voxel_grid(event_input_torch, height=img_lq.shape[0],
                                                        width=img_lq.shape[1])
