...
def paired_SDE_paths_from_folder(folders, keys, filename_tmpl):
    ...
    paths = []
    for in_name, gt_name in zip(input_paths, gt_paths):
        in_path = osp.join(input_folder, in_name)  # 直接按 index 对应
        gt_path = osp.join(gt_folder, gt_name)
        paths.append({
            f'{input_key}_path': in_path,
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    # --- MODIFICATION START ---
    # 1. Scan and sort both folders to get full paths
    input_paths = sorted(list(scandir(input_folder, full_path=True)))
    gt_paths = sorted(list(scandir(gt_folder, full_path=True)))

    # 2. Check if the number of files is the same after sorting
    assert len(input_paths) == len(gt_paths), (
        f'{input_key} and {gt_key} datasets have different number of images: '
        f'{len(input_paths)} vs {len(gt_paths)}.')

    # 3. Pair them by sorted index using zip
    paths = []
